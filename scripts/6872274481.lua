...
local function requesturl(url, bypass) 
    ...
    local repourl = bypass and "https://raw.githubusercontent.com/NTDCore/" or "https://raw.githubusercontent.com/NTDCore/Future/main/"
...
local function getasset(path)
	if not betterisfile(path) then
		local req = requestfunc({
			Url = "https://raw.githubusercontent.com/NTDCore/Future/main/"..path:gsub("Future/assets", "assets"),
			Method = "GET"
		})
...
runcode(function()
    local part
    funnyFly = GuiLibrary.Objects.CombatWindow.API.CreateOptionsButton({
        Name = "FunnyFly",
        Function = function(callback)
            if callback then
...
do 
    local Anti = {["Enabled"] = false}
    Anti = GuiLibrary["Objects"]["MovementWindow"]["API"].CreateOptionsButton({
        ["Name"] = "Anticheat bypass(shit version)",
        ["Function"] = function(callback) 
            if callback then 
          ...
          spawn(function()
            repeat
              if (not getgenv().cower) then
                ...
                local toplay = lplr.Character.Humanoid.Animator:LoadAnimation(AnimC)
                toplay:Play(0.1,1,1)
              until (not getgenv().cower)
            end)
          end
            end
        end
...
runcode(function()
	...
	FastDrop = GuiLibrary.Objects.RenderWindow.API.CreateOptionsButton({
		["Name"] = "Dupe",
		["Function"] = function(callback)
			if callback then
